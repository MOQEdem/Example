...
public abstract class ForgeResourceUpgrader : InteractiveZone
{
    ...
    [SerializeField] private MeshRenderer _forgeModel;
    ...
    private void ApplyNewResource()
    {
        foreach (var stage in _allStages.Stages)
        {
            if (stage.Level == _status.GetCurrentUpgradeLevel())
            {
                _resourceProducer.SetResourcePrefab(stage.Resorce);
                _forgeModel.material.SetTextureOffset("_MainTex", stage.MaterialOffset);
            }
        }
    }
}
